...
% 以下为仿真场景设置
La_near = R_near*lambda/cos(sq_ang)^2/D;
La_far = R_far*lambda/cos(sq_ang)^2/D;
...
% 方针目标设置
Rpt = [R_near+200 R_near+500 R_near+800];
Ypt = [0 0 0];
La = Rpt*lambda/cos(sq_ang)^2/D;
Tc = -Rpt*tan(sq_ang)/V;
Npt = length(Rpt);
...
figure,imagesc(row,col,real(sig)) ;axis xy
...
close(h);
figure,imagesc(abs(f_back)),title('升采样率为10时成像结果'),xlabel('距离维'),ylabel('方位维');